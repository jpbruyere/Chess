...
namespace Crow
{
	public class ChessBoardWidget : GraphicObject
	{
		Picture sprites;
		ChessPiece[,] board;

		[XmlAttributeAttribute()][DefaultValue(null)]
		public virtual ChessPiece[,] Board {
			get { return board; }
			set {
				if (board == value) {
		...
		public ChessBoardWidget ():base()
		{
			sprites = new SvgPicture ("#Chess.ChessPieces.svg");
		}
		protected override void onDraw (Cairo.Context gr)
		{
			...
			r.Inflate (-(int)Math.Max(fe.Height,fe.MaxXAdvance));
			double gridSize;
			double gridX = r.X;
			double gridY = r.Y;
			...
			Fill white = new SolidColor (Color.Ivory);
			white.SetAsSource (gr);
			gr.Rectangle (gridX, gridY, gridSize, gridSize);
			gr.Fill ();

			double cellSize = gridSize / 8;

			Fill black = new SolidColor (Color.Gray);
			black.SetAsSource (gr);
			for (int x = 0; x < 8; x++) {
				for (int y = 0; y < 8; y++) {
					if ((x + y) % 2 != 0)
			...
			for (int x = 0; x < 8; x++) {
				for (int y = 0; y < 8; y++) {
					...
					string spriteName;
					if (board [x, y].Player.Color == ChessColor.White)
						spriteName = "w";
					else
						spriteName = "b";
					...
					sprites.Paint (gr,new Rectangle(
